...
void CDMesh::draw(int (*maptex_callback)())
{
	...
	if (maptex_callback) maptex_callback();////added


	glEnableClientState( GL_VERTEX_ARRAY );
	glVertexPointer(3, GL_FLOAT, 0, &vertices[0][0] );

	wireframe = false;////added

	if ( wireframe ) {
		vector<short> lines(triangles.size()*3*2);
		for ( int i=0; i<triangles.size(); i++ ) {
	...
	glDisableClientState( GL_TEXTURE_COORD_ARRAY );///added
}
