...
void CDFaceData::draw( const vec3& center, const vec3& fitSize )
{
	...
	float scale = min(min(targetSize.x/size.x, targetSize.y/size.y), targetSize.z/size.z);
	if ( isnan(scale) ) {
	...
	glRotatef(yaw*30, 0,1,0); //////////////// -30~~+30

	//maptex_callback();/////added
	mesh.draw(maptex_callback);
