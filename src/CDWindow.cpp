...
static const string CANDIDE_FILE_PATH = "../data/candide3.wfm";
...
static void sliderYaw(Fl_Widget* widget, void* userData)
{
	CDWindow* cdWindow = static_cast<CDWindow*>(userData);
	if ( cdWindow ) {
		Fl_Slider* slider = dynamic_cast<Fl_Slider*>(widget);
		cdWindow->changeYaw( slider->value() );
	}
}
...
: Fl_Window(w, h, label)
{
	...
	if ( suNames.size()>0 ) {
		...
		Fl_Slider* slider = new Fl_Slider( w-30, 10, 20, h-60, "Mouth bending" );
		slider->labeltype(FL_NO_LABEL);
		slider->align(FL_ALIGN_BOTTOM);
		slider->callback(&sliderCallback, this );
		slider->value(0);
		slider->bounds(1,0);
	}
	{
		Fl_Slider* slider = new Fl_Slider( w-60, 10, 20,h-60, "yaw" );
		slider->labeltype(FL_NO_LABEL);
		slider->align(FL_ALIGN_BOTTOM);
		slider->callback(&sliderYaw, this );
		slider->value(0);
		slider->bounds(-1,1);
	}
	resizable(this);
	...
	faceWindow = new CDFaceWindow(10,10,w-100,h-20, faceData);
...
void CDWindow::changeYaw( double newValue )
{
	CDLog << "changeYaw";
	faceWindow->yaw(newValue);
	faceWindow->redraw();
}
